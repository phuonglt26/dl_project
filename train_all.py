...
def train_model(
    model, train_loader, teacher_model, num_epochs, lr
):
    ...
    for epoch in range(num_epochs):
        ...
        print(
            f"Epoch {epoch + 1}, Loss: {avg_loss:.4f}, train_Accuracy: {accuracy * 100:.2f}%"
        )
...
def train_phase():
    ...
    for phase_num in range(1, 11):
        ...
        # Load the train
        preprocessed_data = np.load(data_dir + f"phase_{phase_num}.npy")
        # Load the labels
        labels = np.load(data_dir + f"label_phase_{phase_num}.npy")
        ...
        validation_data = np.load(f"data/preprocessed_val/validation.npy")
        ...
        filename = f"results/result_{phase_num}.txt"
        with open(filename, "w", encoding="utf-8") as file:
            img_names = sorted(
                [name for name in os.listdir("data/Val/")], key=lambda x: int(x.split(".")[0])
            )
